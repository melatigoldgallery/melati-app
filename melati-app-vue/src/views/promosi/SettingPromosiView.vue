  ...
  <div class="container-fluid py-3">
    <div class="d-flex align-items-center justify-content-between mb-3">
      <h4 class="fw-bold mb-0">
        <i class="bi bi-broadcast me-2 text-warning"></i>Setting Promosi
      </h4>
    ...
    <div class="card border-0 shadow-sm mb-3">
      <div class="card-header bg-white fw-semibold small py-2">
        <i class="bi bi-gear me-1 text-warning"></i>Pengaturan Display
      </div>
      <div class="card-body">
        <div class="row g-2">
          <div class="col-md-3">
            <label class="form-label small">Interval Slide (detik)</label>
            <input v-model.number="settings.slideInterval" type="number" min="5" max="120" class="form-control form-control-sm" />
          </div>
          <div class="col-md-3">
        ...
        <div class="mt-2">
          <button class="btn btn-warning btn-sm" @click="saveSettings" :disabled="savingSettings">
            ...
            <i v-else class="bi bi-floppy me-1"></i>Simpan Pengaturan
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="card border-0 shadow-sm">
      <div class="card-header bg-white d-flex justify-content-between align-items-center py-2">
        <span class="fw-semibold small"><i class="bi bi-images me-1 text-warning"></i>Konten Slide</span>
      ...
      <div class="table-responsive">
        <table class="table table-sm mb-0">
          ...
          <tbody>
            <tr v-if="loadingContent"><td colspan="5" class="text-center py-4"><div class="spinner-border spinner-border-sm text-warning"></div></td></tr>
            <tr v-else-if="slides.length === 0"><td colspan="5" class="text-center text-muted py-4">Belum ada konten.</td></tr>
            <tr v-for="slide in sortedSlides" :key="slide.id">
              ...
              <td><span class="badge bg-info text-dark">{{ slide.contentType }}</span></td>
              ...
              <td class="text-center">
                <div class="btn-group btn-group-sm">
                  <button class="btn btn-outline-primary btn-sm" @click="openEdit(slide)"><i class="bi bi-pencil"></i></button>
                  <button class="btn btn-outline-danger btn-sm" @click="deleteSlide(slide)"><i class="bi bi-trash"></i></button>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="row g-2">
              <div class="col-md-6">
                <label class="form-label small">Judul <span class="text-danger">*</span></label>
              ...
              <div v-if="slideForm.contentType === 'HTML'" class="col-12">
                ...
                <textarea v-model="slideForm.htmlContent" rows="4" class="form-control form-control-sm font-monospace" placeholder="<h1>Hello</h1>"></textarea>
              </div>
              ...
              <div v-if="slideForm.contentType === 'Gambar' || slideForm.contentType === 'Video'" class="col-12">
                ...
                <div v-if="editSlide?.fileUrl" class="small text-muted mt-1">
                  File saat ini: <a :href="editSlide.fileUrl" target="_blank">Lihat</a>
                </div>
          ...
          <div class="modal-footer">
            ...
            <button class="btn btn-warning btn-sm" @click="saveSlide" :disabled="savingSlide">
              <span v-if="savingSlide" class="spinner-border spinner-border-sm me-1"></span>Simpan
            </button>
          </div>
        </div>
      </div>
...
const settings = ref({ slideInterval: 30, transitionEffect: "fade", autoPlay: true, showControls: true, enableAnimation: true });
...
async function saveSettings() {
  ...
  } catch (e) { showError("Gagal menyimpan pengaturan", e.message); }
...
function openAdd() {
  ...
  slideForm.value = { title: "", description: "", contentType: "HTML", order: (sortedSlides.value.length + 1), isActive: true, htmlContent: "" };
...
function openEdit(slide) {
  ...
  selectedFile.value = null;
  uploadProgress.value = 0;
  slideForm.value = { title: slide.title, description: slide.description || "", contentType: slide.contentType, order: slide.order, isActive: slide.isActive ?? true, htmlContent: slide.htmlContent || "" };
  Modal.getOrCreateInstance(document.getElementById("slideModal")).show();
}
...
async function saveSlide() {
  if (!slideForm.value.title.trim()) return toast("Judul wajib diisi", "warning");
  ...
  try {
    ...
    if (slideForm.value.contentType === "HTML") {
      ...
      const { fileUrl, filePath } = await uploadFile(selectedFile.value);
    ...
    toast(`Slide berhasil ${editSlide.value ? "diperbarui" : "ditambahkan"}`);
    await loadSlides();
  } catch (e) { showError("Gagal menyimpan slide", e.message); }
  finally { savingSlide.value = false; uploadProgress.value = 0; }
}
