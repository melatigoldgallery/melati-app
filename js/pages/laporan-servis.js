...
function determineJenisData(item) {
  ...
  return item.detailBarangCustom && Array.isArray(item.detailBarangCustom) && item.detailBarangCustom.length > 0
    ? "custom"
    : "servis";
}
...
const smartCache = {
  ...
  get(key) {
    console.warn("⚠️ Using deprecated smartCache. Switch to smartServisCache from servis-service.js");
...
function updateSummaryCards() {
  ...
  const completedCount = filteredServisData.filter((item) => item.statusServis === "Sudah Selesai").length;
  document.getElementById("completedServis").textContent = completedCount;

  const pendingCount = filteredServisData.filter((item) => item.statusServis === "Belum Selesai").length;
  document.getElementById("pendingServis").textContent = pendingCount;

  const takenCount = filteredServisData.filter((item) => item.statusPengambilan === "Sudah Diambil").length;
...
async function deleteDisplayedData() {
  ...
  try {
    ...
    showAlert("success", `Berhasil menghapus ${idsToDelete.length} data servis dan ${storagePaths.length} foto`);
  } catch (error) {
...
async function exportToExcel() {
  ...
  try {
    exportBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i> Mengunduh foto...';
    ...
    headers.push("Status Servis", "Status Pengambilan", "Handle Pengambilan", "Waktu Pengambilan", "Link Bukti Foto");
    ...
    // Note: Direct Excel export (ZIP not possible due to CORS)
    exportBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i> Membuat Excel...';

    // Note: Photos included as URLs in Excel due to CORS restrictions
    const photoCount = filteredServisData.filter((item) => item.buktiPengambilanUrl).length;

    // Generate and download Excel (not ZIP - CORS limitation)
    exportBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i> Menyimpan Excel...';
    XLSX.writeFile(workbook, `Laporan_Servis_${monthNames[month]}_${year}.xlsx`);

    // Show success message with photo info
    let message = `File Excel berhasil diunduh`;
    if (photoCount > 0) {
      message += `<br><small class="text-muted">📷 ${photoCount} link foto disertakan di kolom "Link Bukti Foto".<br>Klik link untuk melihat/download foto.</small>`;
    }

    showAlert("success", message);
  } catch (error) {
    console.error("Error exporting Excel:", error);
    showAlert("danger", "Terjadi kesalahan saat mengekspor Excel: " + error.message);
  } finally {
