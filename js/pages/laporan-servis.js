...
function applyFilters() {
  ...
  const statusFiltered = currentServisData.filter((item) => {
    return (
      (!statusServis || item.statusServis === statusServis) &&
      (!statusPengambilan || item.statusPengambilan === statusPengambilan) &&
...
function exportToExcel() {
  ...
  try {
    ...
    if (jenisData === 'servis') {
      ...
      headers.push('Panjang', 'Warna', 'DP', 'Ongkos')
    }
    headers.push('Status Servis', 'Status Pengambilan', 'Handle Pengambilan', 'Waktu Pengambilan')
    ...
    const worksheetData = [
      ...
      ...filteredServisData.map((item, index) => {
        ...
        if (jenisData === 'servis') {
          ...
          rowData.push(item.panjang || '-', item.warna || '-', item.totalDp || 0, item.ongkos || 0)
        }
...
function exportToPDF() {
  ...
  try {
    ...
    if (jenisData === 'servis') {
      ...
      pdfHeaders.push('Pjg', 'Wrn', 'DP', 'Ongkos')
      widths.push('auto', 'auto', 'auto', 'auto')
    }
    pdfHeaders.push('Status', 'Ambil', 'Handle', 'Waktu')
    widths.push('auto', 'auto', 'auto', 'auto')
