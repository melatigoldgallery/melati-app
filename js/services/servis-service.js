...
export async function getServisByMonthWithFilters(month, year, statusServis = "", statusPengambilan = "") {
  try {
    ...
    // Hitung last day of month secara dinamis
    const lastDay = new Date(year, month, 0).getDate();
    const startDate = `${year}-${month.toString().padStart(2, "0")}-01T00:00:00.000Z`;
    const endDate = `${year}-${month.toString().padStart(2, "0")}-${lastDay.toString().padStart(2, "0")}T23:59:59.999Z`;
...
export async function getServisByMonth(month, year, statusPengambilan = "all") {
  try {
    ...
    // Hitung last day of month secara dinamis (support 28/29/30/31 hari)
    const lastDay = new Date(year, month, 0).getDate();

    // Gunakan ISO string format dengan waktu untuk match data yang disimpan sebagai ISO timestamp
    // Start: awal hari pertama bulan ini (00:00:00.000)
    // End: akhir hari terakhir bulan ini (23:59:59.999)
    const startDate = `${year}-${month.toString().padStart(2, "0")}-01T00:00:00.000Z`;
    const endDate = `${year}-${month.toString().padStart(2, "0")}-${lastDay.toString().padStart(2, "0")}T23:59:59.999Z`;
    ...
    console.log(`✓ Loaded ${servisData.length} servis records (${querySnapshot.size} reads) for ${month}/${year}`);
    return servisData;
  } catch (error) {
