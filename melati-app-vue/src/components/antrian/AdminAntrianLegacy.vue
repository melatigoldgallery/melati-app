...
import {
  ...
  resetQueue,
  writeAnalyticsEntry,
} from "@/services/antrian-service-legacy";
import {
...
async function initClosingSettings() {
  try {
    await ensureClosingAnnouncementSettings(activeFloor.value);
    closingSettings.value = await fetchClosingAnnouncementSettings(activeFloor.value);
  } catch (error) {
...
async function confirmServed() {
  ...
  try {
    ...
    state.value = await nextQueue(state.value, activeFloor.value);
...
async function confirmSkip() {
  ...
  try {
    const q = skipLetter.value + padNumber(skipNumber.value);
    state.value = await addToSkipList(state.value, q, activeFloor.value);
    skipNumber.value = 1;
  } catch (e) {
    console.error(e);
  } finally {
...
async function confirmDelay() {
  ...
  try {
    const q = currentQueueStr.value;
    const s = await addToDelayedQueue(state.value, q, activeFloor.value);
    state.value = await nextQueue(s, activeFloor.value);
  } catch (e) {
    console.error(e);
  } finally {
    busy.value = false;
...
async function confirmCustom() {
  ...
  try {
    state.value = await setCustomQueue(state.value, customLetter.value, customNumber.value, activeFloor.value);
  } catch (e) {
    console.error(e);
  } finally {
    busy.value = false;
...
async function confirmMoveToMissed() {
  ...
  try {
    state.value = await moveToMissed(state.value, q, activeFloor.value);
  } catch (e) {
    console.error(e);
  } finally {
    busy.value = false;
...
async function confirmMissedHandle() {
  ...
  try {
    state.value = await removeFromMissed(state.value, q, activeFloor.value);
    await writeAnalyticsEntry({ queueNumber: q, status: "served", floorId: activeFloor.value });
  } catch (e) {
    console.error(e);
  } finally {
