...
document.getElementById('loginForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    } else if (username === 'admin' && password === 'adminmelati') {
      console.log('Using admin custom credentials')
      const auth = getAuth()
      try {
        await signInWithEmailAndPassword(auth, 'melatigoldshopid@gmail.com', 'svmlt116')
        result = { success: true, role: 'admin_custom', username: 'admin' }
        console.log('Admin custom login successful')
      } catch (authError) {
        console.error('Admin custom auth failed:', authError)
      }
